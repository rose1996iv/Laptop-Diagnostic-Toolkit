...
def write_html_report(report: DiagnosticReport, path: Path) -> None:
    ...
    body = f"<!doctype html><html><head><meta charset='utf-8'><title>Laptop Diagnostic Report</title><style>body{{font:15px Segoe UI,Arial;margin:2rem;color:#17202a}}table{{border-collapse:collapse;width:100%}}th,td{{border-bottom:1px solid #d7dee5;padding:.7rem;text-align:left}}.pass{{color:#147d4c}}.fail{{color:#b42318}}.warning{{color:#9a6700}}.not-available{{color:#667085}}</style></head><body><h1>Laptop Diagnostic Report</h1><p>{html.escape(data['timestamp'])} | Score: <strong>{data['score']}/100</strong> | {html.escape(data['overall_status'])}</p><table><tr><th>Check</th><th>Status</th><th>Summary</th></tr>{rows}</table><h2>Details</h2><pre>{html.escape(json.dumps(data['results'], indent=2, default=str))}</pre></body></html>"
